            ...
            for vec_length in vector_length_list:
                ...
                for normalize_par, clip_par in product([True, False], [True, False]):
                    if (not normalize_par) and (not clip_par):
                        sparsity_ref = 1 - (1 - distance.cosine(vector_this, np.ones_like(vector_this))) ** 2
                    elif (not normalize_par) and clip_par:
                        sparsity_ref = 1 - (1 - distance.cosine(vector_this_nn, np.ones_like(vector_this))) ** 2
